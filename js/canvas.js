...
(function () {
  ...
  function fitWatermarkNames() {
    document.querySelectorAll('.share-card-mini').forEach(function(card) {
      var span = card.querySelector('.share-card-watermark span');
      if (!span) return;
      var target = card.offsetWidth * 0.88;
      span.style.fontSize = '10px';
      var natural = span.offsetWidth;
      if (natural > 0) {
        var size = (target / natural) * 10;
  ...
  function debouncedFit() {
    ...
    resizeTimer = setTimeout(fitWatermarkNames, 100);
  }
  window.addEventListener('resize', debouncedFit);
  window.addEventListener('orientationchange', function() { setTimeout(fitWatermarkNames, 200); });
  ...
  function initShareCards() {
    ...
    cards.forEach((card) => {
      ...
      function draw() {
        ...
        const x1 = sw * (0.3 + Math.sin(t * 0.4) * 0.2);
        const y1 = sh * (0.2 + Math.sin(t * 0.3) * 0.15);
        const x2 = sw * (0.7 + Math.cos(t * 0.35) * 0.2);
        const y2 = sh * (0.8 + Math.cos(t * 0.25) * 0.15);
        ...
        const g1 = ctx.createRadialGradient(x1, y1, 0, x1, y1, sw * 0.7);
        ...
        const g2 = ctx.createRadialGradient(x2, y2, 0, x2, y2, sw * 0.6);
  ...
  function showPage(id) {
    ...
    setTimeout(() => {
      ...
      if (id === 'your-score') { initShareCards(); fitWatermarkNames(); }
