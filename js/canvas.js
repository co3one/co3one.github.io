...
(function () {
  ...
  function showPage(id) {
    ...
    // If navigating from home or overlay not available, do instant switch
    if (!overlay) {
      pages.forEach((p) => { p.classList.remove('active'); p.classList.remove('entered'); });
    ...
    // Phase 1: Fade to dark (particles still visible underneath)
    overlay.classList.add('visible');

    setTimeout(() => {
      // Phase 2: Switch pages while dark
      pages.forEach((p) => { p.classList.remove('active'); p.classList.remove('entered'); });
      ...
      setTimeout(() => {
        // Phase 3: Fade overlay away, revealing new page
        overlay.classList.remove('visible');

        // Phase 4: Trigger staggered hero + content entrance
