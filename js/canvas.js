...
(function () {
  ...
  /* ==========================================================
     2b. SHARE CARD GRADIENT (lazy init — waits for page visible)
     ========================================================== */
  let shareCardStarted = false;

  function initShareCard() {
    if (shareCardStarted) return;
    const shareCanvas = document.getElementById('share-card-canvas');
    if (!shareCanvas) return;
    shareCardStarted = true;

    const sctx = shareCanvas.getContext('2d');
    const dpr = window.devicePixelRatio || 1;
    const sw = 280, sh = 380;
    shareCanvas.width = sw * dpr;
    shareCanvas.height = sh * dpr;
    shareCanvas.style.width = sw + 'px';
    shareCanvas.style.height = sh + 'px';
    sctx.scale(dpr, dpr);
    ...
    function drawShareCard() {
      ...
      const x1 = sw * (0.3 + Math.sin(st * 0.4) * 0.2);
      const y1 = sh * (0.2 + Math.sin(st * 0.3) * 0.15);
      const x2 = sw * (0.7 + Math.cos(st * 0.35) * 0.2);
      const y2 = sh * (0.8 + Math.cos(st * 0.25) * 0.15);

      sctx.fillStyle = '#0a0c14';
      sctx.fillRect(0, 0, sw, sh);

      const g1 = sctx.createRadialGradient(x1, y1, 0, x1, y1, sw * 0.7);
      const a1 = 0.25 + Math.sin(st * 0.5) * 0.08;
      g1.addColorStop(0, `rgba(196, 152, 80, ${a1})`);
      g1.addColorStop(0.5, `rgba(160, 110, 60, ${a1 * 0.4})`);
      g1.addColorStop(1, 'rgba(10, 12, 20, 0)');
      sctx.fillStyle = g1;
      sctx.fillRect(0, 0, sw, sh);

      const g2 = sctx.createRadialGradient(x2, y2, 0, x2, y2, sw * 0.6);
      const a2 = 0.18 + Math.cos(st * 0.45) * 0.06;
      g2.addColorStop(0, `rgba(60, 140, 160, ${a2})`);
      g2.addColorStop(0.5, `rgba(40, 90, 120, ${a2 * 0.4})`);
      g2.addColorStop(1, 'rgba(10, 12, 20, 0)');
      sctx.fillStyle = g2;
      sctx.fillRect(0, 0, sw, sh);

      const x3 = sw * (0.5 + Math.sin(st * 0.2 + 2) * 0.3);
      const y3 = sh * (0.5 + Math.cos(st * 0.15 + 1) * 0.3);
      const g3 = sctx.createRadialGradient(x3, y3, 0, x3, y3, sw * 0.5);
      const a3 = 0.1 + Math.sin(st * 0.6 + 3) * 0.04;
      g3.addColorStop(0, `rgba(140, 70, 90, ${a3})`);
      g3.addColorStop(1, 'rgba(10, 12, 20, 0)');
      sctx.fillStyle = g3;
      sctx.fillRect(0, 0, sw, sh);

      const g4 = sctx.createRadialGradient(sw / 2, sh / 2, 0, sw / 2, sh / 2, sw * 0.8);
      g4.addColorStop(0, 'rgba(255, 255, 255, 0.01)');
      g4.addColorStop(1, 'rgba(0, 0, 0, 0.1)');
      sctx.fillStyle = g4;
      sctx.fillRect(0, 0, sw, sh);
  ...
  function showPage(id) {
    ...
    setTimeout(() => {
      ...
      // Init share card canvas when your-score page first becomes visible
      if (id === 'your-score') initShareCard();
  ...
  if (overlay) {
    ...
    wlForm?.addEventListener('submit', (e) => {
      ...
      // Send email via mailto as fallback (opens mail client)
      // In production, replace with API call
      const subject = encodeURIComponent('miji Waitlist');
      const body = encodeURIComponent('Please add me to the miji waitlist.\n\nEmail: ' + email);
      window.open('mailto:waitlist@co3.one?subject=' + subject + '&body=' + body, '_blank');
